import { defineComponent, resolveComponent, mergeProps, useSSRContext, ref } from "vue";
import "hookable";
...
const _sfc_main$5 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--]--></div><p class="text-slate-500 dark:text-slate-400 mt-1">Más de 500+ pacientes atendidos</p></div></div></div><div class="hero-image opacity-0 relative lg:h-[600px] flex items-center justify-center"><div class="absolute inset-0 glass-card rounded-[2rem] transform rotate-3 scale-105 z-0"></div><div class="relative z-10 w-full h-full glass-card rounded-[2rem] overflow-hidden group"><img${ssrRenderAttr("src", _imports_0$1)} alt="Dra. Yissel Mendoza" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy"><div class="absolute bottom-8 -left-6 glass-card px-6 py-4 flex items-center gap-4 animate-float"><div class="w-12 h-12 rounded-full bg-accent/20 flex items-center justify-center text-accent">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "lucide:shield-check",
        class: "w-6 h-6"
      }, null, _parent));
      _push(`</div><div><p class="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Certificada por el</p><p class="font-display font-bold text-slate-900 dark:text-white">Consejo Médico</p></div></div></div></div></div></div></section>`);
    };
  }
});
...
const _sfc_main$4 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div><p class="ml-3 text-slate-600 dark:text-slate-300"> Egresada del <strong>Centro Médico Nacional de Occidente</strong>. </p></li><li class="flex items-start"><div class="flex-shrink-0 mt-1">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "lucide:award",
        class: "w-5 h-5 text-accent"
      }, null, _parent));
      _push(`</div><p class="ml-3 text-slate-600 dark:text-slate-300"> Cuenta con <strong>certificaciones del consejo médico</strong> vigente. </p></li></ul><div class="pt-6 border-t border-slate-200 dark:border-slate-800 flex flex-wrap gap-4"><div class="glass px-4 py-2 rounded-lg inline-flex items-center space-x-2">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "lucide:file-text",
        class: "w-4 h-4 text-primary dark:text-secondary"
      }, null, _parent));
      _push(`<span class="text-sm font-medium text-slate-700 dark:text-slate-200">Cédula Prof: <strong>12193559</strong></span></div><div class="glass px-4 py-2 rounded-lg inline-flex items-center space-x-2">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "lucide:file-badge",
        class: "w-4 h-4 text-primary dark:text-secondary"
      }, null, _parent));
      _push(`<span class="text-sm font-medium text-slate-700 dark:text-slate-200">Cédula Esp: <strong>14201115</strong></span></div></div></div></div></div></div></div></section>`);
    };
  }
});
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(services, (service, i) => {
        ...
        _push(`</div><h3 class="text-2xl font-bold text-slate-800 dark:text-white mb-4">${ssrInterpolate(service.title)}</h3><p class="text-slate-600 dark:text-slate-300 leading-relaxed">${service.description ?? ""}</p></div>`);
      });
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(faqs, (faq, index) => {
        ...
        _push(`</div></button><div style="${ssrRenderStyle(activeIndex.value === index ? null : { display: "none" })}" class="px-6 pb-6 text-slate-600 dark:text-slate-300 leading-relaxed origin-top"><div class="pt-2 border-t border-slate-100 dark:border-slate-800"><p>${faq.answer ?? ""}</p></div></div></div>`);
      });
      ...
      _push(`</div><div><h3 class="font-bold text-slate-900 dark:text-white mb-2">Protocolo de Emergencias</h3><p class="text-sm text-slate-600 dark:text-slate-300"> Contamos con una línea especial destinada exclusivamente a situaciones médicas urgentes de pacientes ya valorados: <br class="hidden sm:block"><a href="tel:3441041492" class="font-bold text-red-600 dark:text-red-400 hover:underline mt-2 inline-block">344-104-14-92</a>. </p></div></div></div></div></section>`);
    };
  }
});
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      }, _attrs))}><div class="container mx-auto px-4 relative z-10"><div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center"><div class="space-y-8 contact-item opacity-0"><div><h2 class="text-3xl md:text-4xl font-display font-bold text-slate-900 dark:text-white mb-4">Agenda tu cita</h2><p class="text-lg text-slate-600 dark:text-slate-300"> Estamos aquí para ayudarte. Contáctanos para programar una valoración integral de tu salud. </p></div><div class="space-y-6"><div class="flex items-start"><div class="w-12 h-12 rounded-xl bg-primary/10 dark:bg-secondary/10 flex items-center justify-center flex-shrink-0 mt-1">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "lucide:map-pin",
        class: "w-6 h-6 text-primary dark:text-secondary"
      }, null, _parent));
      _push(`</div><div class="ml-4"><h3 class="text-lg font-bold text-slate-900 dark:text-white">Ubicación</h3><p class="text-slate-600 dark:text-slate-300 mt-1"> Hospital MAC SUR (Dentro del Centro Oncológico). <br>República de Perú 102, Torre C, consultorio 201. <br>Aguascalientes, Ags. C.P. 20234. </p></div></div><div class="flex items-start"><div class="w-12 h-12 rounded-xl bg-primary/10 dark:bg-secondary/10 flex items-center justify-center flex-shrink-0 mt-1">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "lucide:phone",
        class: "w-6 h-6 text-primary dark:text-secondary"
      }, null, _parent));
      _push(`</div><div class="ml-4"><h3 class="text-lg font-bold text-slate-900 dark:text-white">Teléfonos</h3><div class="mt-1 space-y-1"><p class="text-slate-600 dark:text-slate-300"> Citas: <a href="tel:4494970240" class="font-bold text-primary dark:text-secondary hover:underline">449-497-02-40</a></p><p class="text-sm text-slate-500 dark:text-slate-400"> Emergencias (Uso restringido): <a href="tel:3441041492">344-104-14-92</a></p></div></div></div></div><div class="glass-card p-6 border-l-4 border-l-secondary mt-12 bg-white/60 dark:bg-slate-800/60"><div class="flex text-yellow-500 mb-3"><!--[-->`);
      ssrRenderList(5, (i) => {
        _push(ssrRenderComponent(_component_Icon, {
          name: "lucide:star",
          class: "w-4 h-4 fill-current",
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    useHead({
      ...
      meta: [
        { name: "description", content: "Médico internista en Aguascalientes." }
      ]
    });
    return (_ctx, _push, _parent, _attrs) => {
...
//# sourceMappingURL=index-D52p_mEW.js.map
